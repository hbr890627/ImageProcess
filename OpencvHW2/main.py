...
def ResNet():
    rndClass = np.random.randint(2)
    rndPic = np.random.randint(0, 10000)
    print(rndClass)
    print(rndPic)

    cla = ["Cat", "Dog"]
    # print(cla[rndClass])
    # print("sample/test1/"+cla[rndClass]+"/"+str(rndPic)+".jpg")
    rndImg = cv2.imread("PetImages/"+cla[rndClass]+"/"+str(rndPic)+".jpg")

    if rndImg is not None:
        cv2.imshow(cla[rndClass], rndImg)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    else:
        while rndImg is None:
            print("not found a image")
            rndImg = cv2.imread(
                "PetImages/"+cla[rndClass]+"/"+str(rndPic)+".jpg")
        cv2.imshow(cla[rndClass], rndImg)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def ResNet_Predict():
    ...
    rndPic = np.random.randint(0, 10000)
    cla = ["Cat", "Dog"]
    rndImg = image.load_img(
        "PetImages/"+cla[rndClass]+"/"+str(rndPic)+".jpg", target_size=(224, 224))
    x = image.img_to_array(rndImg)
    x = np.expand_dims(x, axis=0)
    pred = net.predict(x)[0]
    top_inds = pred.argsort()[::-1][:5]
    print("\n"+cla[rndClass]+"/"+str(rndPic)+".jpg")
    for i in top_inds:
        print('{}% {}'.format(int(pred[i]*100), class_list[i]))

    # net = load_model('model-resnet50-final.h5')
    # class_list = ['cats', 'dogs']
    # files = glob.glob('PetImage/Cat/*.jpg')
...
def AccuracyCompar():
    ...
    plt.bar(0, 0.69, 0.1)
    plt.bar(1, 0.6, 0.1)
    plt.ylabel("accuracy")          # 設定y軸標題
    plt.xticks(index, ('not resize', 'resize'))
